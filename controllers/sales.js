...
const { successResponse, errorResponse, generateSaleId } = require('../helpers/utility')

/**
 * CREATE MULtiPLE SALES
 * @param {sales array containing sale objects} req
 * @param {*} res
 */
...
const createMultipleSales = async (req, res) => {
	try {
		...
		for (let i = 0; i < sales.length; i++) {
			const { amount, products } = sales[i]
			const insertData = { saleNumber: generateSaleId(), amount, products, userId: req.user.id }
			const createdSale = await DB.sales.create(insertData)
			if (createdSale) {
				successArr.push({ successMsg: `Sale placed successfully!` })
			} else {
				errorArr.push({ successMsg: `An error occured`, data: sales[i] })
			}
		}
...
const updateSale = async (req, res) => {
	try {
		...
		await sale.update({ saleStatus: status ? status : sale.saleStatus })
...
/**
 * GET SALES FROM DATABASE, CALLED FROM SALES LISTING PAGE
 * @param {*} req
 * @param {*} res
 */
...
const getSales = async (req, res) => {
	...
	try {
		...
		if (!sales.length) return successResponse(res, `No sale available!`, [])
		return successResponse(res, `${sales.length} sale${sales.length > 1 ? 's' : ''} retrived!`, sales)
	} catch (error) {
...
module.exports = { getSaleDetail, createMultipleSales, createSale, updateSale, getSales }
